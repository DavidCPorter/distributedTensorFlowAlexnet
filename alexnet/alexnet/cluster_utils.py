...
function start_cluster() {
    if [ -z $2 ]; then
        ...
        if [ "$2" = "single" ]; then
            ...
            nohup ssh node-0 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=0" > serverlog-0.out 2>&1&
            nohup ssh node-1 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=1" > serverlog-1.out 2>&1&
            nohup ssh node-2 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=2" > serverlog-2.out 2>&1&
            nohup ssh node-3 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=3" > serverlog-3.out 2>&1&
        fi
    fi
}
